...
import ebikecity.project.config.EBCConfigGroup;

public class EBCSmoothingTravelTimeModule extends AbstractModule {
	@Override
	public void install() {
	...
	public SmoothingTravelTime provideSmoothingTravelTime(Network network, TravelTimeCalculatorConfigGroup config,
			EBCConfigGroup ebcConfig,
			ControlerConfigGroup conConfig) {
		...
		double smoothingIncreasingAlpha = ebcConfig.getTravelTimeEstimationAlpha(); // 9;
		double smoothingDecreasingAlpha = ebcConfig.getTravelTimeEstimationAlpha();
