...
//[2]   path of network file (xml or xml.gz)

public class AnalysisTravelTimeFromEvents {
	
	public static void main(String[] args) {
		
		EventsManager eventsManager = EventsUtils.createEventsManager();
		
		AnalysisTravelTimeEventsHandler evHandler = new AnalysisTravelTimeEventsHandler();
		
		eventsManager.addHandler(evHandler);
		
		new MatsimEventsReader(eventsManager).readFile(args[0]);
		
		evHandler.writeTravelTimes(args[1], args[2]);
