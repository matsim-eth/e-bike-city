...
import ebikecity.utils.probabilisticrouting_dev.SpeedyGraph.LinkIterator;
 ...
 */
public class StochasticDijkstra implements LeastCostPathCalculator {
	...
	public StochasticDijkstra(SpeedyGraph graph, TravelTime tt, TravelDisutility td) {
		this.graph = graph;
		this.tt = tt;
		this.td = td;
...
//			ArrayList<Double> travelUtils = new ArrayList<>();
//			this.outLI.reset(nodeIdx);
//			while (this.outLI.next()) {
//				int linkIdx = this.outLI.getLinkIndex();
//				Link link = this.graph.getLink(linkIdx);
//				travelUtils.add(this.td.getLinkTravelDisutility(link, currTime, person, vehicle));
//			}
//	        double minTravelUtils = Collections.min(travelUtils);
//	        double maxTravelUtils = Collections.max(travelUtils);
//	        double randomFactor = Math.random() * (maxTravelUtils - minTravelUtils) * 0.5;
	        
//	        double standardDeviation = 0.001 * Math.sqrt(travelUtils.stream().mapToDouble(val -> val).map(val -> Math.pow(val - travelUtils.stream().mapToDouble(Double::doubleValue).average().getAsDouble(), 2)).average().getAsDouble());
//	        double mean = travelUtils.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
//	        mean = 1;
//	        standardDeviation = 3;
	        
//	        double draw = Math.abs(random.nextGaussian() * standardDeviation + mean); //
//	        System.out.println("draw: " + draw);
//	        draw = 1;
	        
	        double mean = 0;
	        double standardDeviation = 0.2;
	        double factorDraw = Math.abs(random.nextGaussian() * standardDeviation + mean);
//	        System.out.println("factor draw: " + factorDraw);
			...
			while (this.outLI.next()) {
				...
				double newCost = currCost + (this.td.getLinkTravelDisutility(link, currTime, person, vehicle) * (1 + factorDraw));
//				System.out.println("[" + runningIndex + "] " + "traveltime: " + travelTime + " newtime: " + newTime + " newCost: " + newCost);
